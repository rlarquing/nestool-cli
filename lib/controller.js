...
const {escribirFichero, escribirIndex, escapeRegExp, buscarFichero, direccionFichero} = require("../util/util");
const fs = require("fs");
...
const createController = async (controller) => {
    ...
    try {
        ...
        re = escapeRegExp('$tag');
        templateController = templateController.replace(re, nombre + 's'
        );

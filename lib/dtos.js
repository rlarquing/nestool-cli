...
const {generarDto, generarReadDto, generarDtoPadre, generarReadDtoPadre} = require("../util/entityToDtoParser");
...
const createCrudDto = async (newCrudDto) => {
    ...
    try {
        ...
        dto = generarDto(entityPath);
        if(newCrudDto.moduleName==='nomenclator'){
            let padrePath = ruta.normalize(`${path}/entity/generic-nomenclador.entity.ts`);
            dtoPadre = generarDtoPadre(padrePath);
            console.log(dtoPadre)
            dto.validadores=dtoPadre.validadores+dto.validadores;
            dto.atributos=dtoPadre.atributos+dto.atributos;
        }
        ...
        dto = generarReadDto(entityPath,newCrudDto.moduleName);
        if (dto.import !== '') {
        ...
        if(newCrudDto.moduleName==='nomenclator'){
            let padrePath = ruta.normalize(`${path}/entity/generic-nomenclador.entity.ts`);
            dtoPadre = generarReadDtoPadre(padrePath,newCrudDto.moduleName);
            dto.atributos=dtoPadre.atributos+dto.atributos;
            dto.parametros=dtoPadre.parametros+dto.parametros;
            dto.thisAtributos=dtoPadre.thisAtributos+dto.thisAtributos;
        }

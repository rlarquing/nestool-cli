...
const {escribirFichero, escribirIndex, escapeRegExp, quitarSeparador, aInicialMayuscula, aInicialMinuscula,
    removeFromArr
} = require("../util/util");
const fs = require("fs");
...
const createService = async (service) => {
    ...
    try {
        ...
        let modulePath = ruta.normalize(`${path}/${service.moduleName}.module.ts`);
        let mod = parseM(modulePath);
        let restoClase = mod.restoClase.join('');
        for (let importacion of mod.import) {
            if (importacion.indexOf(`./service'`) !== -1) {
                let parametros = importacion.substring(importacion.indexOf('{') + 1, importacion.indexOf('}'));
                mod.import[mod.import.findIndex((item) => item === importacion)] = importacion.replace(parametros, parametros + ', ' + quitarSeparador(service.entityName,'-')+'Service');
            }
            if (importacion.indexOf(`.service';`) !== -1) {

    ...
    if (pendiente.length > 0) {
        for (const entidad of pendiente) {
            ...
            let imTmp = parseFile.import.join("");
                ...
                if (!imTmp.includes(importacionEntidad) || !imTmp.includes(`import {${quitarSeparador(entityName, '-')}Entity} from '../../${moduleName}/entity';\n`)) {
                    if (entidad[0] !== quitarSeparador(entityName, '-') + 'Entity') {
                        if (dir.includes(moduleName)) {
                            parseFile.import.push(importacionEntidad);
                        } else {
                            parseFile.import.push(`import {${quitarSeparador(entityName, '-')}Entity} from '../../${moduleName}/entity';\n`);
                        }
                    }
                }

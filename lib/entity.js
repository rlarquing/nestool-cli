...
const {
    ...
    generarColumna, transformar, direccionClassBusquedaInterna, entidades, entidadesR, esquemas, quitarSeparador,
    formatearArchivos
} = require("../util/util");
...
const createNewEntity = async (entity) => {
    ...
    try {
        ...
        if (!fs.existsSync(filePath)) {
            ...
            procesarPendientes(pendiente, importacionEntidad, entity.entityName, entity.moduleName);
        }
...
const incorporarAttrEntity = async (entity) => {
    ...
    if (fs.existsSync(filePath)) {
        ...
        if (tmp.constructor.length > 0) {
            entidad = entidad.concat(tmp.constructor.join(''));
            entidad = entidad.concat(tmp.restoClase.join(''));
        } else {
            entidad = entidad.concat('constructor(nombre: string, descripcion: string, $parametros) {super(nombre, descripcion);$thisAtributos}');
            entidad = entidad.concat(tmp.restoClase.join(''));
        }
    ...
    let atrConst = [];
    try {
        procesarPreguntas(answers, importaciones, nombre, direccion, dirModulo, atributo, atributos, typeorm, atrConst, quitarSeparador(entity.entityName, '-') + 'Entity', entity.moduleName, pendiente);
        let thisAtributos = [];
        for (const atr of atrConst) {
            thisAtributos.push(transformar(atr, '%campo:%tipo', 'this.%campo=%campo;'))
        }
        ...
        typeorm = eliminarDuplicado(typeorm);
        let tmpOrm = []
        for (const item of typeorm) {
            if (entidad.includes(item) !== true) {
                tmpOrm.push(item);
            }
        }
        entidad = entidad.replace("$typeorm", tmpOrm.toString());
...
const procesarPendientes = (pendiente, importacionEntidad, entityName, moduleName) => {
    if (pendiente.length > 0) {
        for (const entidad of pendiente) {
            ...
            let importEntidad = entityName.includes('Entity') ? entityName : quitarSeparador(entityName, '-') + 'Entity';
            if (!imTmp.includes(importacionEntidad) || !imTmp.includes(`import {${importEntidad}} from '../../${moduleName}/entity';\n`)) {
                if (entidad[0] !== quitarSeparador(entityName, '-') + 'Entity') {
                    if (dir.includes(moduleName)) {
                        parseFile.import.push(importacionEntidad);
                    } else {
                        parseFile.import.push(`import {${importEntidad}} from '../../${moduleName}/entity';\n`);
                    }
                }
            }
            ...
            if (parseFile.atributos.length !== 0) {
                ...
                if (parseFile.clase.substring(parseFile.clase.length - 1) === ';') {
                    parseFile.atributos.push(entidad[1]);
                } else {
            ...
            fichero = fichero.concat(parseFile.constructor);
            fichero = fichero.concat(parseFile.restoClase);
            fichero=fichero.join('\n');
            fichero=fichero.replace('$parametros','');
            fichero=fichero.replace('$thisAtributos','');
            console.log(fichero);

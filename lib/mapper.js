    ...
    try {
        ...
        importarArr.forEach((imp) => {
            if (imp.includes('../entity')) {
                tmpEntidad.push(imp.substring(imp.indexOf('{') + 1, imp.indexOf('}')).trim().replace(',', ''));
            } else if (imp.includes('../dto')) {
                tmpDto.push(imp.substring(imp.indexOf('{') + 1, imp.indexOf('}')).trim().replace(',', ''));
            } else if (imp.includes('../repository')) {
                tmpRepository.push(imp.substring(imp.indexOf('{') + 1, imp.indexOf('}')).trim().replace(',', ''));
            } else {
                resto.push(imp);
            }
        });
        resto.push(`import {${tmpEntidad.toString()}} from "../entity"`);
        resto.push(`import {${tmpDto.toString()}} from "../dto"`);
        resto.push(`import {${tmpRepository.toString()}} from "../repository"`);
        ...
        let modulePath = ruta.normalize(`${path}/${mapper.moduleName}.module.ts`);
...
const atributos = (path, entityName, moduleName) => {
    ...
    if (impModulo.size > 0) {
        for (const key of impDto.keys()) {
            ...
            let dto = eliminarDuplicado(impDto.get(key).dto);
            let repository = eliminarDuplicado(impDto.get(key).repository);
            let mapper = eliminarDuplicado(impDto.get(key).mapper);
...
const analisisentityToDto = (path, entityName) => {
    ...
    for (const item of analisis) {
        if (item.includes('@ManyToOne') || item.includes('@OneToOne')) {
            ...
            if (esNomenclador) {
                analisisentityToDto.push(`const ${readDto}: ReadNomencladorDto = this.nomencladorGenericMapper.entityToDto($attrName.${findElemento(parametros, entidad + 'Entity').toString().split(':')[0]});`)
            } else {
                analisisentityToDto.push(`const ${readDto}: Read${entidad}Dto = this.${aInicialMinuscula(entidad)}Mapper.entityToDto($attrName.${findElemento(parametros, entidad + 'Entity').toString().split(':')[0]});`);
            }
        }
        if (item.includes('@ManyToMany') && item.includes('@JoinTable')) {
            let tmp = item.split('=>');

...
const atributos = (mapper) => {
    ...
    for (const item of atributosR) {
        let entidad = aInicialMinuscula(eliminarSufijo(item.kind, 'Entity'));
...
const parametrosDtoToEntity = (entityName) => {
    ...
    for (const parametro of clase.attributes) {
        if (parametro.kind.includes('Entity')) {
            parametrosDtoToEntity.push(parametro.name);
        } else {
            parametrosDtoToEntity.push(`create$nameDto.${parametro.name}`)
        }
    }
...
const analisisDtoToUpdateEntity = (entityName) => {
    ...
    for (const item of atributosR) {
        ...
        if (item.relation === '@ManyToOne' || item.relation === '@OneToOne') {
            if (esNomenclador) {
                analisisDtoToUpdateEntity.push(`const ${item.name}:${aInicialMayuscula(entidad) + 'Entity'} = await this.nomencladorGenericRepository.findById(NomencladorTypeEnum.${entidad.toLocaleUpperCase()}, create$nameDto.${item.name});`);
            } else {
                analisisDtoToUpdateEntity.push(`const ${item.name}:${aInicialMayuscula(entidad) + 'Entity'} = await this.${entidad}Repository.findById(create$nameDto.${item.name});`);
            }
        }
        if (item.relation === '@ManyToMany') {
            if (esNomenclador) {
                analisisDtoToUpdateEntity.push(`const ${item.name}:${aInicialMayuscula(entidad) + 'Entity[]'} = await this.nomencladorGenericRepository.findByIds(NomencladorTypeEnum.${entidad.toLocaleUpperCase()}, create$nameDto.${item.name});`);
            } else {
                analisisDtoToUpdateEntity.push(`const ${item.name}:${aInicialMayuscula(entidad) + 'Entity[]'} = await this.${entidad}Repository.findByIds(create$nameDto.${item.name});`);
            }
        }
        entidad = '';
    }
    ...
    for (const parametro of clase.attributes) {
        if (parametro.kind.includes('Entity')) {
            analisisDtoToUpdateEntity.push(`update$nameEntity.${parametro.name} = ${parametro.name};`);
        } else {
            analisisDtoToUpdateEntity.push(`update$nameEntity.${parametro.name} = update$nameDto.${parametro.name};`)
        }
    }
...
const parametrosEntityToDto = (entityName) => {
    ...
    for (const parametro of clase.attributes) {
        if (parametro.kind.includes('Dto') || parametro.name==='dtoToString') {
            parametrosEntityToDto.push(parametro.name);
        } else {
            parametrosEntityToDto.push(`$attrNameEntity.${parametro.name}`)
        }
    }
